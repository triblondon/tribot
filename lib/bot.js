...
var Bot = function(options) {
	...
	Object.keys(options.plugins).forEach(function(id) {
		self.loadPlugin(id, options.plugins[id]);
	});
};
...
var enableDebug = function() {
	...
	var origSend = this.client.send;
	self.client.send = function(stanza) {
		console.log('OUT > ' + stanza.root().toString());
		return origSend.call(self.client, stanza);
	};
	console.log("Debug enabled.  I/O will be echoed to console");
};
...
var onStanza = function(stanza) {
	...
	if (stanza.is('message') && stanza.attrs.type) {
		...
		if (this.options.debug) console.log('IN  < Message from '+stanza.attrs.from+': '+body);
		var data = {
		...
		if (this.options.debug) console.log('IN  < Invitation to join '+inviteRoom+' from '+inviteSender);
		...
		if (stanza.attrs.type === 'result') {
			if (this.options.debug) console.log("IN  < IQ reply (#"+stanza.attrs.id+"): "+stanza.root().toString());
		...
		if (from.getResource() && from.getDomain() === this.options.mucHost) {
			this.rooms[from.getLocal()] = this.rooms[from.getLocal()] || {};
			this.rooms[from.getLocal()][from.getResource()] = status;
		}
		// TODO: Read <x xmlns="http://jabber.org/protocol/muc#user">/<item>.name/nick for profile info
		// TODO: Read <show> for status
		if (this.options.debug) console.log('IN  < '+stanza.attrs.from+' is '+status);

	} else {

		this.emit('unhandleddata', stanza);
		if (this.options.debug) console.log("IN  < "+stanza.root().toString());
	}
};
...
// Connects the bot to the server and sets the XMPP event listeners. This
...
Bot.prototype.connect = function() {
	...
	this.client.on('online', function() {
		...
		self.keepalive = setInterval(function() {
			self.client.send(new ltx.Element('message', {}));
			self.emit('sendping');
		}, 30000);
		self.options.rooms.forEach(self.join.bind(self));
		self.emit('connect');
	});
...
Bot.prototype.leave = function(room) {
	var packet = new ltx.Element('presence', { type: 'unavailable', from: this.options.jid, to: room + '@' + this.options.mucHost + '/' + this.name })
		.c('x', { xmlns: 'http://jabber.org/protocol/muc' })
		.c('status').t('hc-leave')
	;
